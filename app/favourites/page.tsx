...
// Mock: first 4 products are favourited by default for demo
const MOCK_FAVOURITE_IDS = [1, 2, 3, 4];

export default function FavouritesPage() {
  const allProducts: Product[] = productsData as Product[];

  const [favouriteIds, setFavouriteIds] = useState<number[]>(MOCK_FAVOURITE_IDS);

  const favourites = allProducts.filter(p => favouriteIds.includes(p.id));
